...
public class dataBaseApp {
    ...
    public void getAllContactsUsingWebRowSetAndParseItToXML() {
        try {
            // connect to database
            RowSetFactory myRSF = RowSetProvider.newFactory();
            RowSet myRS = myRSF.createWebRowSet();
            ...
            // processing result
            try {
                FileOutputStream myXML = new FileOutputStream("contacts.xml");
                try {
                    ((WebRowSet) myRS).writeXml(myXML);
                } catch (IOException e) {
